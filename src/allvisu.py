...
class AllVisu:
    ...
    def decohistomap(func):
        ...
        def inner_hm(self, **kwargs):
            ...
            if type(geopdwd) != gpd.geodataframe.GeoDataFrame:
                ...
                #if func.__name__ == 'pycoa_mapfolium' or func.__name__ == 'pycoa_map' or func.__name__ == 'innerdecomap' or func.__name__ == 'innerdecopycoa_eo':
                if func.__name__ in ['pycoa_mapfolium','pycoa_map','pycoa_eo' ,'pycoa_pimpmap','pycoa_mpltmap']:
                    if isinstance(input['where'].iloc[0],list):
                        geom = self.kindgeo
                        geodic={}
    ...
    def pycoa_mapfolium(self, **kwargs):
        ...
        mapa = folium.Map(tiles=tile, attr='<a href=\"http://pycoa_fr\"> ©pycoa_fr </a>' + msg)
    ...
    def decopycoa_eo(func):
        @wraps(func)
        def innerdecopycoa_eo(self,**kwargs):
            geopdwd = kwargs.get('geopdwd')
            input_field = kwargs.get("input_field")
            geopdwd['cases'] = geopdwd[input_field]
            loca=geopdwd['where'].unique()

            if self.pycoa_eopandas:
